...
# przerobka r-deve-san-clang
# https://raw.githubusercontent.com/rocker-org/r-devel-san-clang/master/Dockerfile
# na wersje z rstudio
...
### od tego miejsca wlasna tworczosc z wybudowaniem rstudio:

RUN apt-get update && apt-get install -y libxml2 libxml2-dev libssl-dev vim wget gdebi-core libapparmor1 libclang-11-dev libclang-dev libpq5 psmisc sudo git libgit2-dev

RUN wget https://download2.rstudio.org/server/bionic/amd64/rstudio-server-1.4.1106-amd64.deb
RUN apt-get install -y lsb-release
RUN dpkg -i rstudio-server-1.4.1106-amd64.deb
...
RUN R -e 'options(repos = "https://cloud.r-project.org") ; install.packages(c("XML", "httr", "curl","dplyr", "tidyr", "rmarkdown", "maps",  "climate", "roxygen2", "data.table", "devtools"))'

# download climate dev
WORKDIR /home/rstudio
RUN git clone https://github.com/bczernecki/thundeR.git --branch new_pietrek
